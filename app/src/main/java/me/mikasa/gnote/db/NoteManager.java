...
public class NoteManager {
    ...
    public void initTable(){
        SPUtil.putString(mContext,"category","所有");
        try {
            ...
            SPUtil.putInt(mContext,"noteId",contents.length);
            ContentValues values=new ContentValues();
            values.put("id",1);
            values.put("category","所有");//所有--1
            values.put("pos",String.valueOf(0));
            database.insert(noteDBHelper.TABLE_CATEGORY,null,values);
            values.clear();
            values.put("id",2);
            values.put("category","日常");//所有--1
            values.put("pos",String.valueOf(1));
            database.insert(noteDBHelper.TABLE_CATEGORY,null,values);
            SPUtil.putInt(mContext,"categoryNum",2);//category已有一个“所有”分类和一个"日常"分类
    ...
    public void updateCategory(Category category){
        try {
            ...
            if (!TextUtils.isEmpty(category.getPos())){
                values.put("pos",category.getPos());
            }
            //根据id来update
